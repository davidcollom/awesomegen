...
func writeByTopTags(b *strings.Builder, list config.List) {
	...
	if list.TopicGroupingMode == config.TopicGroupingModeNested {
		...
		b.WriteString(TOCHeader)
		for _, pk := range prim {
			fmt.Fprintf(b, "- [%s](#%s)\n", TitlecaseKeys(pk, list.Locale), slug(pk))
		}
		b.WriteString("\n")

		for _, pk := range prim {
			fmt.Fprintf(b, "## %s\n\n", TitlecaseKeys(pk, list.Locale))
			sec := nested[pk]
			sk := mapsSortedKeys(sec)
			for _, s := range sk {
				if s != list.TopicFallback || len(sec) > 1 {
					fmt.Fprintf(b, "### %s\n\n", TitlecaseKeys(s, list.Locale))
				}
	...
	b.WriteString(TOCHeader)
	for _, k := range keys {
		fmt.Fprintf(b, "- [%s](#%s)\n", TitlecaseKeys(k, list.Locale), slug(k))
	}
	b.WriteString("\n")

	for _, k := range keys {
		fmt.Fprintf(b, "## %s\n\n", TitlecaseKeys(k, list.Locale))
